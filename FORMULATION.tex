...
\journal{IJNME}
\usepackage{lmodern,mathpazo,amsmath,amsfonts,amssymb,siunitx,graphicx,diagbox,floatrow,booktabs,gnuplot-lua-tikz,structmech,subcaption}
%\geometry{showframe}
...
\newcommand*\patchMath[1]{%
\patchAmsMathEnvironmentForLineno{#1}%
\patchAmsMathEnvironmentForLineno{#1*}}%
\AtBeginDocument{\patchMath{equation}\patchMath{align}\patchMath{flalign}\patchMath{alignat}\patchMath{gather}\patchMath{multline}}
...
\node[align=center]at(17.5,8){mapping $\mathbold{T}$};
\node[align=center]at(42.5,8){mapping $\mathbold{D}$};
...
%\node[align=center]at(5,5){deformed\\configuration\\$\mathbold{X}$};
\node[align=center]at(5,5){undeformed\\configuration\\$\mathbold{X}$};
\node[align=center]at(30,5){intermediate\\configuration\\$\mathbold{x}_m$};
\node[align=center]at(55,5){deformed\\configuration\\$\mathbold{x}$};
\node[align=center]at(5,-15){deformed\\configuration\\$\mathbold{x}$};
\node[align=center]at(30,-15){translation\\$\mathbold{u}_t$};
\node[align=center]at(55,-15){distortion\\$\mathbold{u}_d$};
...
\SetKw{Update}{update}
\SetKw{Break}{break}
\KwIn{$\Delta\mathbold{q}$, $\mathbold{q}_n$, $\mathbold{\alpha}_n$, $\mathbold{\beta}_n$, $\mathbold{\zeta}_n$, $\mathbold{V}_n$, $\mathbold{W}_n$}
\KwOut{$\mathbold{K}$, $\mathbold{R}$, $\mathbold{q}_{n+1}$, $\mathbold{\alpha}_{n+1}$, $\mathbold{\beta}_{n+1}$, $\mathbold{\zeta}_{n+1}$, $\mathbold{V}_{n+1}$, $\mathbold{W}_{n+1}$}
$\mathbold{q}_{n+1}=\mathbold{q}_n+\Delta\mathbold{q}$\;
$\Delta\mathbold{\zeta}=-\mathbold{V}^{-1}_n\mathbold{M}^\mT\mathbold{\alpha}_n-\mathbold{V}^{-1}_n\mathbold{W}_n^\mT\Delta\mathbold{q}$\tcp*[r]{counter: 12 \eqsref{eq:answer_zeta}}
$\Delta\mathbold{\beta}=\tilde{\mathbold{N}}\Delta\mathbold{q}+\tilde{\mathbold{M}}\Delta\mathbold{\zeta}$\tcp*[r]{counter: 143 \eqsref{eq:answer_beta}}
$\mathbold{\zeta}_{n+1}=\mathbold{\zeta}_n+\Delta\mathbold{\zeta}$\;
$\mathbold{\beta}_{n+1}=\mathbold{\beta}_n+\Delta\mathbold{\beta}$\;
\ForAll{integration points}{
$\mathbold{\varepsilon}_{n+1}^i=\mathbold{\phi}_{\varepsilon}^i\mathbold{\beta}_{n+1}$\tcp*[r]{counter: 33}
obtain $\tilde{\mathbold{E}}_{n+1}^i$ and $\tilde{\mathbold{\sigma}}_{n+1}^i$ from the material model\;
assemble $\displaystyle\tilde{\mathbold{H}}_{n+1}=\sum{}w^i\mathbold{\phi}_{\varepsilon}^{i,\mT}\tilde{\mathbold{E}}_{n+1}^i\mathbold{\phi}_{\varepsilon}^i$\tcp*[r]{counter: 462}
assemble $\displaystyle\tilde{\mathbold{S}}_{n+1}=\sum{}w^i\mathbold{\phi}_\varepsilon^{i,\mT}\tilde{\mathbold{\sigma}}_{n+1}^i$\tcp*[r]{counter: 33}
}
$\mathbold{\alpha}_{n+1}=\mathbold{H}^{-\mT}\tilde{\mathbold{S}}_{n+1}$\tcp*[r]{counter: 121 \eqsref{eq:answer_alpha}}
update $\mathbold{U}_{n+1}$, $\mathbold{V}_{n+1}$ and $\mathbold{W}_{n+1}$ using $\tilde{\mathbold{H}}_{n+1}$\tcp*[r]{counter: 1850 \eqsref{eq:answer_uvw}}
$\mathbold{K}=\mathbold{U}_{n+1}-\mathbold{W}_{n+1}\mathbold{V}^{-1}_{n+1}\mathbold{W}_{n+1}^\mT$\tcp*[r]{counter: 1716 \eqsref{eq:answer_stiffness}}
$\mathbold{R}=\mathbold{N}\mathbold{\alpha}_{n+1}-\mathbold{W}_{n+1}\mathbold{V}_{n+1}^{-1}\mathbold{M}^\mT\mathbold{\alpha}_{n+1}$\tcp*[r]{counter: 144 \eqsref{eq:answer_resistance}}
...
\end{figure}
Numerical results are shown in \tabref{table:cooks}. Similar to the previous curved beam, GCMQ shows a good coarse mesh accuracy. For a \num{4x4} mesh grid, GCMQ can produce the best result compared to all existing four-node membranes. Since the distribution of the end force is not given, a uniformly distributed pattern is assumed. Better results can be obtained by averaging tip deflections for dense mesh grids.
\begin{table}[htb]
\centering\scriptsize
